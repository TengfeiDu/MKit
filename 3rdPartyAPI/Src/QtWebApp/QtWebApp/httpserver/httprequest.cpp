...
QJsonObject HttpRequest::toJson() const
{
    QJsonObject json;
    QString ls,ps,cs;
    QMultiMap<QByteArray, QByteArray>::const_iterator i = headers.constBegin();
    while (i != headers.constEnd()) {
        ls += QString("%1 = %2;   ").arg(i.key().data()).arg(i.value().data());
        ++i;
    }
    i = parameters.constBegin();
    while (i != parameters.constEnd()) {
        ps += QString("%1 = %2;   ").arg(i.key().data()).arg(i.value().data());
        ++i;
    }
    i = cookies.constBegin();
    while (i != cookies.constEnd()) {
        cs += QString("%1 = %2;   ").arg(i.key().data()).arg(i.value().data());
        ++i;
    }
    json.insert("headers",ls);
    json.insert("body",getBody().data());
    json.insert("method",getMethod().data());
    json.insert("path",getPath().data());
    json.insert("rawPath",getRawPath().data());
    json.insert("status",getStatus());
    json.insert("version",getVersion().data());
    json.insert("parameters",ps);
    json.insert("peerAddress",getPeerAddress().toString());
    json.insert("cookies",cs);
    return json;
}
